...
#'
#' @examples
#' Time_plot_cor(project_name = 'test', num_question_Est = 12, type_theta = 'Hier', path = '/Users/ap/Desktop', 
#' parameter1 = 'beta', parameter2 = 'r')
Time_plot_cor <- function(project_name,
                          num_question_Est,
                          type_theta, 
                          path, 
                          parameter1, 
  ...
  p1 <- matrix(unlist(stan[parameter1]), nrow = dim(stan$beta)[1], byrow = F)
  p2 <- matrix(unlist(stan[parameter2]), nrow = dim(stan$beta)[1], byrow = F)
