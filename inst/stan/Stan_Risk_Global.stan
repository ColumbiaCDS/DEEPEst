...
model{
  ...
  global_theta~ lognormal(0.5, 1);
  for(p in 1:npart) {
      choices[p] ~ bernoulli_logit(global_theta * (v2[p] - v1[p]));
  }
}
generated quantities{
  ...
  for(p in 1:npart) {
    for(i in 1:nchoice){
      postpred[p,i] = bernoulli_logit_rng(global_theta * (v2[p,i] - v1[p,i]));
      choiceper = inv_logit(global_theta * (v2[p, i] - v1[p, i]));
